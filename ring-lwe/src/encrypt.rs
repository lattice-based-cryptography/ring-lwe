...
pub fn encrypt(
    ...
    let ct0 = polyadd(polyadd(polymul(pk[0].clone(), u.clone(), q, poly_mod), e1, q, poly_mod),scaled_m,q,poly_mod);
    let ct1 = polyadd(polymul(pk[1].clone(), u.clone(), q, poly_mod), e2, q, poly_mod);
